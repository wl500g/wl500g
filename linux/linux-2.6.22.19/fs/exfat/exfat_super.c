...
#endif
{
	...
	if (flags & (LOOKUP_CREATE | LOOKUP_RENAME_TARGET))
		return 0;
#else
	if (!(flags & (LOOKUP_CONTINUE | LOOKUP_PARENT))) {
		if (flags & (LOOKUP_CREATE /*| LOOKUP_RENAME_TARGET*/))
			return 0;
	}
#endif
...
static int exfat_cmpi(const struct dentry *parent, struct qstr *str,
		const struct qstr *name)
{
	...
	blen = exfat_striptail_len(str);
	if (alen == blen) {
		if (t == NULL) {
			if (strncasecmp(name->name, str->name, alen) == 0)
				return 0;
		} else if (nls_strnicmp(t, name->name, str->name, alen) == 0)
			return 0;
	}
	return 1;
}

static int exfat_cmp(const struct dentry *parent, struct qstr *str,
		const struct qstr *name)
{
	unsigned int alen, blen;
	...
	blen = exfat_striptail_len(str);
	if (alen == blen) {
		if (strncmp(name->name, str->name, alen) == 0)
			return 0;
	}
	return 1;
}
