...
extern int proc_nr_files(ctl_table *table, int write,
...
static int proc_do_cad_pid(struct ctl_table *table, int write,
		  void __user *buffer, size_t *lenp, loff_t *ppos);
static int proc_dointvec_taint(struct ctl_table *table, int write,
			       void __user *buffer, size_t *lenp, loff_t *ppos);
#endif
...
repeat:
	...
	for ( ; table->ctl_name || table->procname; table++) {
		...
		if (n == table->ctl_name) {
			...
			error = do_sysctl_strategy(table,
						   oldval, oldlenp,
						   newval, newlen);
			return error;
		}
...
#ifdef CONFIG_SYSCTL_SYSCALL_CHECK
	sysctl_check_table(header->ctl_table);
#endif
	spin_lock(&sysctl_lock);
