...
static void __cpuinit uasm_bgezl_hazard(u32 **p, struct uasm_reloc **r, int instance)
{
	switch (instance) {
	case 0 ... 7:
...
static void __cpuinit uasm_bgezl_label(struct uasm_label **l, u32 **p, int instance)
{
	switch (instance) {
	case 0 ... 7:
...
static __cpuinit void build_tlb_write_entry(u32 **p, struct uasm_label **l,
					 struct uasm_reloc **r,
			...
			unsigned int bvaddr, unsigned int ptr)
{
	...
	if (in_compat_space_p(MODULE_START) && !uasm_rel_lo(MODULE_START)) {
	...
	if (in_compat_space_p(modd) && !uasm_rel_lo(modd)) {
	...
	if (in_compat_space_p(swpd) && !uasm_rel_lo(swpd)) {
		uasm_il_b(p, r, label_vmalloc_done);
 ...
 */
static void __cpuinit
build_r3000_tlb_reload_write(u32 **p, struct uasm_label **l, struct uasm_reloc **r,
