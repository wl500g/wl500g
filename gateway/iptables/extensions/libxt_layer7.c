...
static int parse_protocol_file(char * filename, const unsigned char * protoname, struct xt_layer7_info *info)
{
	FILE * f;
	char * line = NULL;
	size_t len = 0;
...
static void
parse_layer7_protocol(const unsigned char *s, struct xt_layer7_info *info)
{
...
static void layer7_check(unsigned int flags)
{
	if (!flags)
		xtables_error(PARAMETER_PROBLEM,
